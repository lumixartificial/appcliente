const SW_VERSION = "v9.2-final"; // Versión actualizada
...
self.addEventListener('install', (event) => {
    console.log(`[SW-CLIENTE ${SW_VERSION}] Instalando y forzando espera de activación.`);
    ...
    event.waitUntil(
        clients.matchAll({ type: 'window', includeUncontrolled: true })
            .then((windowClients) => {
                ...
                if (clientToFocus) {
                    console.log('[SW-CLIENTE] Ventana en URL de destino encontrada y enfocada.');
                ...
                for (let i = 0; i < windowClients.length; i++) {
                    ...
                    if (client.url.startsWith(self.location.origin) && 'focus' in client) {
                        console.log('[SW-CLIENTE] Ventana de la misma app encontrada. Navegando y enfocando.');
                        return client.navigate(targetUrl).then(c => c.focus());
                    }
                }

                // 3. Si no hay ninguna ventana existente que pueda ser reutilizada/enfocada, abre una nueva.
                console.log('[SW-CLIENTE] No hay ventanas existentes para reutilizar. Abriendo una nueva.');
                if (clients.openWindow) {
                    return clients.openWindow(targetUrl);
                } else {
                    console.error('[SW-CLIENTE] clients.openWindow no soportado. No se puede abrir nueva ventana.');
            ...
            .catch(error => {
                if (error instanceof TypeError && error.message.includes("is not the client's active service worker")) {
                    console.warn(`[SW-CLIENTE] Reintento de apertura por error de Service Worker inactivo. Abriendo nueva ventana.`);
                    return clients.openWindow(targetUrl); // Forzar apertura de nueva ventana si hay este error.
                }
                console.error('[SW-CLIENTE] Error en notificationclick:', error);
